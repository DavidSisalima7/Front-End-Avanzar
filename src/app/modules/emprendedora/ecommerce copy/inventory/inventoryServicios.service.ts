...
export class InventoryServiceServicios {
    ...
    /*
    listarServicio(): void {
        this._httpClient.get<InventarioPublicaciones[]>("http://evamarket.ec:8080/api/publicaciones/listaPublicacionesXServicios")
          .subscribe((data) => {
    ...
    listarServicio(): void {
        ...
        this._userService.user$
            ...
            .subscribe((vendedora: Vendedor) => {
                ...
                const url = `http://evamarket.ec:8080/api/publicaciones/listaPublicacionesXServicios/${vendedorId}`;
    ...
    getCategoriesServicio(): Observable<CategoriaServicio[]> {
        return this._httpClient.get<CategoriaServicio[]>('http://evamarket.ec:8080/api/categoriaServicio/listar').pipe(
            tap((categories) => {
    ...
    getCategoriesPublicacion(): Observable<CategoriaPublicacion[]> {
        return this._httpClient.get<CategoriaPublicacion[]>('http://evamarket.ec:8080/api/categoria/listar').pipe(
            tap((categories) => {
    ...
    getPublicacionesServicios(
        ...
        return this._httpClient
            .get<{ pagination: InventoryPagination; servicios: InventarioPublicaciones[] }>('http://evamarket.ec:8080/api/publicaciones/listar', {
                params: params
            })
            .pipe(
    ...
    createPublicacion(publicacion: InventarioPublicaciones, files: any | null): Observable<InventarioPublicaciones> {
        ...
        return this.publicaciones$.pipe(
            take(1),
            switchMap(publicaciones => this._httpClient.post<InventarioPublicaciones>('http://evamarket.ec:8080/api/publicaciones/registrarConFoto', formData, { headers: headers }).pipe(
                map((newPublicacion) => {
    ...
    updatePublicacion(id: number, publicacion: InventarioPublicaciones): Observable<InventarioPublicaciones> {
        return this.publicaciones$.pipe(
            take(1),
            switchMap(publicaciones => this._httpClient.put<InventarioPublicaciones>(`http://evamarket.ec:8080/api/publicaciones/actualizar/${id}`, publicacion
            ).pipe(
    ...
    deletePublicacion(id: number): Observable<boolean> {
        return this._httpClient.put<boolean>(`http://evamarket.ec:8080/api/publicaciones/eliminar/${id}`, null).pipe(
            switchMap(isDeleted => {
                if (isDeleted) {
